...
#include <string>
#include "../Agents/AgentManager.h" // Include AgentManager
#include "../Agents/AStarAgent.h"
#include "../Agents/BFSAgent.h"
#include "../Agents/DijkstraAgent.h"
#include "../Maze/Maze.h"

using namespace std;

// Global AgentManager to manage agents
    ...
    for (auto& rows : toolbox.mazeTiles) {
        for (auto& sprites : rows) {
            if (toolbox.MazeSize > 29 && toolbox.MazeSize <= 59) {
                tilesize = 16;
                if (sprites.getTexture() != toolbox.MazeTileSprite16.getTexture()) {
...
void confirmation() {
    ...
    currentMaze = new Maze(toolbox.MazeSize);
}

void PlaceHolder() {
    // This function is replaced by specific agent button handlers
}

void CreateAStarAgent() {
    if (!currentMaze) return;
    AStarAgent* agent = new AStarAgent(currentMaze->start);
    agentManager.AddAgent(*agent);
    agentManager.RunAgents(*currentMaze);
}

void CreateBFSAgent() {
    if (!currentMaze) return;
    BFSAgent* agent = new BFSAgent(currentMaze->start);
    agentManager.AddAgent(*agent);
    agentManager.RunAgents(*currentMaze);
}

void CreateDijkstraAgent() {
    if (!currentMaze) return;
    DijkstraAgent* agent = new DijkstraAgent(currentMaze->start);
    agentManager.AddAgent(*agent);
    agentManager.RunAgents(*currentMaze);
}

// Update button handlers in Toolbox
void UpdateToolboxButtons() {
    Toolbox& toolbox = Toolbox::getInstance();
    toolbox.AStarButton = new Button(sf::Vector2f(1200, 450), CreateAStarAgent);
    toolbox.BFSButton = new Button(sf::Vector2f(1200, 600), CreateBFSAgent);
    toolbox.DijkstraButton = new Button(sf::Vector2f(1200, 300), CreateDijkstraAgent);

    // Reassign sprites
    toolbox.DijkstraSprite.setPosition(toolbox.DijkstraButton->getPosition());
    toolbox.DijkstraButton->setSprite(&toolbox.DijkstraSprite);
    toolbox.AStarSprite.setPosition(toolbox.AStarButton->getPosition());
    toolbox.AStarButton->setSprite(&toolbox.AStarSprite);
    toolbox.BFSSprite.setPosition(toolbox.BFSButton->getPosition());
    toolbox.BFSButton->setSprite(&toolbox.BFSSprite);
}

// Call this after Toolbox initialization
void InitializeGame() {
